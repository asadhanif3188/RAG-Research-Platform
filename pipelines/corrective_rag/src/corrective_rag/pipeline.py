"""CRAGPipeline — wraps the LangGraph CRAG graph to match the pipeline interface.

Provides the same `run(QueryRequest) -> QueryResponse` interface as the
NaiveRAGPipeline and MultimodalRAGPipeline, enabling seamless API integration.
"""
...
class CRAGPipeline:
    """Corrective RAG pipeline with relevance grading and adaptive retrieval.
    ...
    Args:
        ...
        grading_model: Claude model for grading (defaults to Haiku for cost).
    ...
    def __init__(
        ...
        self._graph: CRAGGraph | None = None
    ...
    def connect(self) -> None:
        ...
        self._graph.connect()
        logger.info("CRAGPipeline fully connected")

    async def run(self, request: QueryRequest) -> QueryResponse:
        """Execute the CRAG pipeline for a single query."""
        ...
        # Convert documents back to RetrievalResult for the response
        sources = [
            RetrievalResult(
